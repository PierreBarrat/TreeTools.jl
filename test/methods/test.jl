...
root_2 = TreeTools.read_newick(
    "$(dirname(pathof(TreeTools)))/../test/methods/tree1_reordered.nwk"
)
...
@testset "Ancestors" begin
    root_1 = TreeTools.read_newick(
        "$(dirname(pathof(TreeTools)))/../test/methods/tree1.nwk"
    )
    ...
    root_2 = TreeTools.read_newick(
        "$(dirname(pathof(TreeTools)))/../test/methods/tree2.nwk"
    )
    @test lca((root_2.child[1].child[1], root_2.child[1].child[2])).label == "ABC"
    @test lca((
        root_2.child[1].child[1].child[1],
        root_2.child[1].child[1].child[2],
        root_2.child[1].child[2],
    )).label == "ABC"
...
@testset "Spanning tree 1" begin
    ...
    @test sort(TreeTools.branches_in_spanning_tree(t, "A", "B")) ==
        sort(["A", "B", "NODE_2"])
...
@testset "Simple rooting" begin
    ...
    tree = parse_newick_string(nwk)
    TreeTools.root!(tree, "B"; time=0.5)
    @test isroot(ancestor(tree["B"]))
    @test pw_distances == [distance(tree, x, y) for x in leaf_labels for y in leaf_labels]
...
@testset "Midpoint rooting" begin
    @testset "1" begin
        ...
        for n in nodes(t)
            @test (n.isroot && ismissing(branch_length(n))) ||
                (!n.isroot && !ismissing(branch_length(n)))
        end
