...
@testset "TreeNode level functions" begin
    root_1 = TreeTools.read_newick(
        "$(dirname(pathof(TreeTools)))/../test/prunegraft/tree1.nwk"
    )
    ...
    @testset "Pruning (node)" begin
        ...
        root_ref = TreeTools.read_newick(
            "$(dirname(pathof(TreeTools)))/../test/prunegraft/tree1_Apruned.nwk"
        )
        @test root == root_ref
        # Pruning with copy
        root = deepcopy(root_1)
    ...
    @testset "Grafting (node)" begin
        ...
        @test root == TreeTools.read_newick(
            "$(dirname(pathof(TreeTools)))/../test/prunegraft/tree_grafttest1.nwk"
        )
    end
    ...
    @testset "Deleting" begin
        ...
        @test temp == root &&
            root == TreeTools.read_newick(
            "$(dirname(pathof(TreeTools)))/../test/prunegraft/tree_del1.nwk"
        )
    end
end
...
@testset "Grafting new node onto tree" begin
    ...
    @testset "2" begin
        E = TreeNode(; label="E", tau=5.0)
        tc = copy(t)
        @test_throws ArgumentError graft!(tc, E, tc["A"])
        graft!(tc, E, tc["A"]; graft_on_leaf=true, time=1.0)
        @test !isleaf(tc["A"])
    ...
    @testset "4" begin
        E = TreeNode(; label="E", tau=4.0, data=MiscData())
        tc = copy(t)
...
@testset "Insert" begin
    ...
    @test branch_length(s) + branch_length(tc["A"]) == 1.0
...
@testset "delete_branches!" begin
    ...
    for cutoff in [1 / k for k in reverse(1:K)]
        tc = copy(tree)
        n_removed = count(n -> !isroot(n) && branch_length(n) < cutoff, internals(tree))
