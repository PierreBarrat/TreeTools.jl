...
function merge!(nodes, ids, τ)
    ...
    for n in nodes[ids]
        TreeTools.graftnode!(r, n; safe=false) # r is a fresh node, no loop possible
    end
    deleteat!(nodes, ids)
    push!(nodes, r)
    return nothing
...
"""
    sample_tree_height(C::Coalescent)

Sample the height of a tree generated by coalescent process `C`.
This simulates the coalescent process once and returns the total height (time to MRCA).
"""
sample_tree_height(C::Coalescent) = sample_tree_height!(deepcopy(C), 0)
function sample_tree_height!(C::Coalescent, H)
    merger_size, τ = choose_event(C) # number of nodes to be coalesced and time to event
    H += τ
    C.n += -merger_size + 1
    if C.n == 1
        ...
        return sample_tree_height!(C, H)
    end
end

# Deprecations for old tree_height functions
@deprecate tree_height(C::Coalescent) sample_tree_height(C)
@deprecate tree_height!(C::Coalescent, H) sample_tree_height!(C, H)
