...
"""
function insert!(
    t::Tree{T}, n::TreeNode; name=get_unique_label(t), time=zero(branch_length(n))
) where {T}
...
"""
delete_null_branches!(tree::Tree; threshold=1e-10) =
    delete_branches!(n -> branch_length(n) < threshold, tree.root)
...
function delete_branches!(f, n::TreeNode; keep_time=false)
    ...
    if !isroot(n) && f(n)
        if isleaf(n)
            ...
            n.tau = ismissing(n.tau) ? missing : 0.0
        else
